# powershell -executionpolicy bypass -File .\03-UserSetup.ps1
...
$removableDevicePath = "HKU:\$userSID\SOFTWARE\Policies\Microsoft\Windows\RemovableStorageDevices"
if (!(Test-Path $removableDevicePath)) {
    New-Item -Path $removableDevicePath -Force
}
New-ItemProperty -Path $removableDevicePath -Name 'Deny_All' -PropertyType Dword -Value 1 -Force

# Prevent the Start Menu Search from providing internet results and using your location
$searchPath = "HKU:\$userSID\SOFTWARE\Microsoft\Windows\CurrentVersion\Search"
if (!(Test-Path $searchPath)) {
    New-Item -Path $searchPath -Force
}
New-ItemProperty -Path $searchPath -Name 'BingSearchEnabled' -PropertyType DWord -Value 0 -Force
New-ItemProperty -Path $searchPath -Name 'AllowSearchToUseLocation' -PropertyType DWord -Value 0 -Force
New-ItemProperty -Path $searchPath -Name 'CortanaConsent' -PropertyType DWord -Value 0 -Force

Write-Host "Access restricted to the domain: $allowedDomain for user $username, settings changes disabled, and Microsoft Store apps disabled, Copilot disabled, Outlook disabled for the user, disable removable device"
